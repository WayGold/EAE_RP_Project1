...
class DeliveryZone:
    ...
    def get_mirror_global_position(self, map_pos_x) :
        return self.mirror_image_x_pos + map_pos_x
    ...
    def detect_mirror_collision(self, container: Container, map: Map):
        # if container is no where near the barrier horizontally, there is no collision so return true immediately to
        # save power
        if container.position_rect.x + container.position_rect.width < self.get_mirror_global_position(map.starting_dx) or container.position_rect.x > self.get_mirror_global_position(map.starting_dx) + self.width:
            return False
